...
import { DataActionType, DataType } from '@/types/event'
...
interface ICheckIsNotChangedOptions {
  type: DataType
...
function checkIsNotChanged(options: ICheckIsNotChangedOptions): boolean {
  // 检查传入的值是否与原值相同
  const { type, action, has_original_value, original_value, method_name, args } = options
...
export const listen = (
  ...
  return (target: any, property_name: string, descriptor: PropertyDescriptor) => {
    ...
    descriptor.value = async function (this: DataObjectType, ...args: any[]) {
      ...
      let is_not_changed = checkIsNotChanged({
        type: this.type,
        action,
        has_original_value,
        original_value,
